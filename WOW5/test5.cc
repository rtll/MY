...
public:
    Warrior(size_t e,size_t s,size_t noc,string species,string f)
        :_weapons(make_shared<multimap<size_t,shared_ptr<Weapon>>>())
    ...
    shared_ptr<multimap<size_t,shared_ptr<Weapon>>> weapons() { return _weapons; }
    ...
    void weapon_sort_before_fight(shared_ptr<multimap<size_t,shared_ptr<Weapon>>> w){
        //因为multimap已对武器编号进行排序
        //故只需对有一个以上Arrow的武士对其使用次数排序
        if(w->count(ARROW) > 1){
            shared_ptr<vector<shared_ptr<Weapon>>> temp_weapons(make_shared<vector<shared_ptr<Weapon>>>());
            //转移到vector排序
    ...
    void weapon_sort_before_rape(shared_ptr<multimap<size_t,shared_ptr<Weapon>>> w){
        if(w->count(ARROW) > 1){
    ...
    int attack(Warrior* me,multimap<size_t,shared_ptr<Weapon>>::iterator weapon_pos,Warrior* enemy){
        //当当前武士没有武器用完两轮或当前武器没有使用次数则无法攻击，直接退出
        if(weapon_pos == me->_weapons->end() || !weapon_pos->second->rest_times()){
            return 0;
        }
        //本方扣掉武器使用次数
    ...
    virtual ~Warrior(){}
private:
    shared_ptr<multimap<size_t,shared_ptr<Weapon>>> _weapons;
    size_t _element;
    size_t _strength;
    size_t _No_of_camp;
    string _species;
